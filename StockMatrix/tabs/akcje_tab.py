...
# --- Poprawne importy z biblioteki ta ---
...
def akcje_tab():
    ...
    last_price = float(close_data.iloc[-1]) if not close_data.empty else 0
    last_rsi = float(df['RSI'].iloc[-1]) if not df['RSI'].empty else 0
    last_macd = float(df['MACD'].iloc[-1]) if not df['MACD'].empty else 0
    last_adx = float(df['ADX'].iloc[-1]) if not df['ADX'].empty else 0
    last_atr = float(df['ATR'].iloc[-1]) if not df['ATR'].empty else 0

    col1.metric("Price (USD)", f"${last_price:.2f}")
    col2.metric("RSI (14)", f"{last_rsi:.2f}")
    col3.metric("MACD", f"{last_macd:.2f}")
    col4.metric("ADX", f"{last_adx:.2f}")
    col5.metric("ATR (14)", f"{last_atr:.2f}")
